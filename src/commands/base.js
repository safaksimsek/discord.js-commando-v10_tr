...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return "**`"+this.name+"`** komutunu sadece **`Bot Sahibi`** kullanabilir.";
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return "**`"+this.name+"`** komutunu kullanabilmen için **`"+permissions[missing[0]]+"`** yetkisine sahip olmalısın.";
				}
				return oneLine`
					**\`${this.name}\`** komutunu kullanabilmen için şu yetkilere sahip olman gerekli:
