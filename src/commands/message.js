...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`**\`${this.command.name}\`** komutu sadece sunucu kanallarında kullanılabilir.`);
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`**\`${this.command.name}\`** komutu sadece **NSFW** kanallarında kullanılabilir.`);
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`**\`${this.command.name}\`** komutunu kullanmak için gerekli izinlere sahip değilsin.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`**\`${this.command.name}\`** komutunu çalıştırabilmem için **"${permissions[missing[0]]}"** yetkisi gerekli.`
					);
				}
				return this.reply(oneLine`
					**\`${this.command.name}\`** komutunu çalıştırabilmem için şu yetki(ler) gerekli:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`**\`${this.command.name}\`** komutunu **${remaining.toFixed(1)}** saniye sonra kullanabilirsin.`
			);
		}
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				return this.reply(stripIndents`
					...
					Lütfen **${ownerList || 'bot sahibine'}**${invite ? `veya **${invite} sunucusuna** bildirin.` : ' bildirin.'}
				`);
			}
		}
	}
